...
public class EditPhieuMuon extends Dialog {
    ...
    public EditPhieuMuon(@NonNull Context context,PhieuMuon phieuMuon,Load_list loadList) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i =0 ; i<sachList.size();i++){
            if(phieuMuon.getMaSach() == (sachList.get(i).getId())){
        ...
        for (int i =0 ; i<thanhVienList.size();i++){
            if(phieuMuon.getMaTV() == (thanhVienList.get(i).getId())){
                spinnerThanhVien.setSelection(i);
                break;
            }
        ...
        date.setText(phieuMuon.getDate());
